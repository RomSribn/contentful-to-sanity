...
function createSanitySchema(typesArray, options = {}) {
  const types = typesArray.map((type) =>
    Object.assign(type, { name: generateValidId(type.name) })
  );
...
function generateSchemaForType(type) {
  ...
  const preparedType = defaultsDeep(
    { preview: { prepare: "__PREPARE__" } },
    type
  );
...
function generateImport(typeName) {
  return `import a${typeName} from './${typeName}'`;
}

function generateValidId(typeName) {
  return +typeName[0] ? `A${typeName}` : upperFirst(typeName);
}
